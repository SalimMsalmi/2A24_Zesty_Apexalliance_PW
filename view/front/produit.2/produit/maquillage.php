...
?>
 <?php
 $maquillage="";
        $products2=$bdd->query('SELECT * FROM produit  where categorie like "maquillage"');
        if(isset($_GET["s"]) AND !empty ($_GET["s"]))
{

   $recherche=htmlspecialchars($_GET['s']);
   $products2=$bdd->query('SELECT * FROM produit where nom like "%'.$recherche.'%" AND categorie="maquillage"');
}
if(isset($GET["Trier"]))
{if ($GET["Trier"]=="Tri par nom")
  $products2=$bdd->query('SELECT * FROM produit order by `nom` DESC AND categorie="maquillage"');
   ...
   <script type="text/javascript">
     $(document).ready(function(){
      ...
      function filterPorducts(){
        ...
        $.ajax({
            url:"price-range-maquillage.php",
            type:"POST",
